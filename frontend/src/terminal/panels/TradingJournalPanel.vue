...
<template>
  <div class="trading-journal-panel">
    ...
    <template v-if="activeTab === 'journal'">
      ...
      <LoadingState v-if="loading && groups.length === 0" type="table" :rows="6" />
...
.status { display: flex; align-items: center; justify-content: center; padding: 20px; color: var(--muted); font-size: var(--font-sm); }

.empty-icon { font-size: var(--font-xl); color: var(--muted); margin: 0; }
.empty-text { font-size: var(--font-sm); color: var(--muted); margin: 0; }
...
.stat-label { font-size: var(--font-xs); color: var(--color-text-tertiary); margin-bottom: 4px; }
...
.table-header {
  ...
  font-size: var(--font-xs); color: var(--color-text-tertiary); text-transform: uppercase; flex-shrink: 0;
}
.table-body { flex: 1; overflow-y: auto; font-size: var(--font-sm); }
...
.breakdown-chip {
  font-size: var(--font-xs); padding: 1px 6px; border-radius: var(--radius-sm); font-weight: 500;
}
