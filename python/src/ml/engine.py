...
class MLService(ml_pb2_grpc.MLServiceServicer):
    ...
    async def RLPredict(self, request, context):
        """Phase 10.3: RL inference — predict action from observation."""
        try:
            # For now, return a hold action (1) as a safe default.
            # Full implementation loads the trained model and performs inference.
            return ml_pb2.RLPredictResponse(
                action=1,
                action_value=0.0,
                action_probs={"0": 0.2, "1": 0.6, "2": 0.2},
            )
        except Exception as e:
            logger.exception("RLPredict failed")
            return ml_pb2.RLPredictResponse()
