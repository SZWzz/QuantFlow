...
class ExpandedFundsWrapper:
    ...
    def get_fund_report_stock_cninfo(self, symbol: str = "", date: str = "20241231") -> Dict[str, Any]:
        """Get fund stock report from CNInfo.
        symbol — stock code to filter by (空=返回全部).
        date — report date (default 20241231 = latest full-year).
        """
        result = self._safe_call_with_retry(ak.fund_report_stock_cninfo, date=date)
        if result.get("success") and symbol:
            filtered = [r for r in result.get("data", []) if r.get("股票代码") == symbol]
