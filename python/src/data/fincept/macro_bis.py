...
class BISAPI:
    ...
    async def get_summary(self) -> Dict[str, Any]:
        """Get latest values for all BIS dataflows (for card display).
        
        Fetches the most recent data point in parallel for each dataflow,
        returns them as {dataflow_id: {latest_value, unit, date}}.
        """
        catalog = await self.get_available_datasets()
        flows = catalog.get("data", {}).get("data", {}).get("dataflows", [])
        if not flows:
            return {"success": False, "error": "No dataflows found", "dataflows": []}

        async def _latest_for_flow(flow_id: str) -> tuple:
            try:
                raw = await self.get_data(flow_id, "all", last_n_observations=1, detail="dataonly")
                sdmx = raw.get("data", {}).get("data", raw.get("data", {}))
                datasets = sdmx.get("dataSets", [])
                ds = datasets[0] if datasets else {}
                reporting_end = ds.get("reportingEnd", "")
                date_str = reporting_end[:10] if reporting_end else ""
                series_map = ds.get("series", {})
                for sk, sv in series_map.items():
                    ...
                    for ok, ov in obs.items():
                        ...
                        if val is not None:
                            ...
                            if not math.isnan(fv):
                                return (flow_id, fv, "", date_str)
                return (flow_id, None, "", "")
            except Exception:
                return (flow_id, None, "", "")

        tasks = [_latest_for_flow(f["id"]) for f in flows]
        ...
        for r in results:
            if isinstance(r, tuple) and len(r) == 4:
                fid, val, unit, date = r
                if val is not None:
                    values[fid] = {"latest_value": val, "unit": unit, "date": date}

        # Merge values into the catalog flows
        enriched = []
        for f in flows:
            entry = {"id": f["id"], "name": f.get("name", f["id"]), "names": f.get("names", {})}
            vid = f["id"]
            if vid in values:
                entry["latest_value"] = values[vid]["latest_value"]
                entry["unit"] = values[vid]["unit"]
                entry["latest_date"] = values[vid]["date"]
            else:
        ...
        return {"success": True, "dataflows": enriched, "timestamp": int(time.time())}
