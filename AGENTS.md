...
cd python && .venv/bin/python -m pytest tests/ -x -q
...
├── app.go                  # Exported Go functions (Wails bindings)
├── app_*.go                # Domain-specific exported bindings (market, research, trading, ...)
└── internal/
    ├── workflow/           #   Workflow engine (Kahn DAG + 200+ nodes under nodes/)
    ├── trading/            #   Trading engine + brokers
    ├── market/             #   MarketDataHub + adapters
    ├── backtest/           #   Backtest engines (CN/HK/US, price limits)
    ├── ai/                 #   AI agent orchestrator
    ├── portfolio/          #   Portfolio + risk
    ├── research/           #   Equity research + sentiment
    ├── notify/             #   Notification engine
    ├── schedule/           #   Cron scheduler
    ├── auth/               #   Credential encryption (AES-256-GCM + OS keychain)
    ├── storage/            #   SQLite (WAL mode)
    ├── python/             #   gRPC bridge to Python
    ├── normalize/          #   Data normalization
    ├── ws/                 #   WebSocket hub
    ├── data/               #   Data lifecycle (archive/clean/import/export)
